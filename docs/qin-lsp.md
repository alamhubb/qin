...
- Qin object member collection should use `QinSourceStructure.ObjectDeclaration.memberDeclarations`. Do not flatten `fields()` and `methods()` inside `QinObjectSymbols`; the source-structure adapter owns member order and duplicate-name handling before consumers bridge ranges back to PSI.
