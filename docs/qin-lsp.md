...
- Unresolved-reference messages should use `QinReferenceElements.referenceName` when rendering the unresolved token name. They must not concatenate raw PSI text for reference identifiers, because reference element normalization belongs to the shared reference helper.
