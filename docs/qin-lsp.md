...
- Source-structure ranges should bridge back to PSI wrappers through shared PSI tree helpers such as `QinPsiTree.elementAtOrParentOfType`. Keep offset-to-wrapper lookup in one platform adapter surface instead of reimplementing `findElementAt` plus parent walking in each resolver.
...
- Qin-owned Go To Declaration coverage should include same-file object/member usages and `this.` member usages through `myFixture.getElementAtCaret()`, not only imported references. This proves the IDEA editor path reaches local `METHOD_NAME` and `FIELD_NAME` PSI nodes.
