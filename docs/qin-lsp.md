...
- PSI element to containing file lookup should go through `QinPsiTree.containingFile` in import/source-structure bridge consumers. `QinImportBindings` must not call `PsiElement.getContainingFile()` directly when it is preparing to bridge source-structure import facts back to PSI.
