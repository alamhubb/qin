...
- Qin module import file resolution should stay inside `QinModuleImportTable`. Consumers such as `QinObjectSymbols` should pass `QinImportBindings.ImportBinding` to the table instead of rebuilding `QinModuleImportTable.QinImport`, because the table owns module-specifier classification and path resolution.
