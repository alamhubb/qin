...
- Java and Qin module import tables should expose element-based constructors such as `fromElement` that bridge through `QinPsiTree.containingFile`. References and diagnostics should not call `element.getContainingFile()` just to build an import table, because each import table owns its module-specifier classification over the containing Qin file.
- Qin module import file resolution should stay inside `QinModuleImportTable`. Consumers such as `QinObjectSymbols` should pass `QinImportBindings.ImportBinding` to the table instead of rebuilding `QinModuleImportTable.QinImport`, because the table owns module-specifier classification and path resolution. `QinModuleImportTable` should store and return `QinImportBindings.ImportBinding` directly, not define a parallel `QinImport` record.
- `QinModuleImportTable` is the allowed owner of `VirtualFile.getParent().findFileByRelativePath(...)` for relative Qin module imports. It should still obtain the importing source file through `QinPsiTree.originalVirtualFile`, but VFS parent traversal and `.qin` extension probing belong to module path resolution, not to the generic PSI tree adapter.
...
- Import binding lookup should bridge a PSI name element to `QinSourceStructure.ImportSpecifierMatch` through `QinPsiTree.importSpecifierMatchAtNameElement`. `QinImportBindings` must not own raw `getTextRange().getStartOffset()` offset lookup for import names.
- Import binding collection should go through `QinPsiTree.importSpecifierMatches`, which delegates to `QinSourceStructure.importSpecifierMatches`. Do not flatten `ImportDeclaration.specifiers()` or call source-structure import match collection directly inside `QinImportBindings`, because source-structure owns import declaration/specifier pairing and `QinPsiTree` owns the PSI-file bridge.
...
- IDEA Find Usages coverage for Java interop should include aliased static member usages such as `G.greet()` and `G.DEFAULT_NAME` from `import { Greeter as G } from "java:demo"`, proving the member token still participates in `ReferencesSearch` while the qualifier remains a local alias reference.
- No-local-parser smoke should guard the critical Java interop Find Usages and Rename platform tests for class, static method, and static field references, including aliased usages such as `G.greet()` and `G.DEFAULT_NAME`. These checks must keep Java PSI refactoring on the Qin `PsiReference` path instead of drifting into LSP fake definitions, reference-only tests, or string rewrites.
...
- Qin PSI-file filtering for editor surfaces should go through `QinPsiTree.isQinFile`. Reference providers, semantic highlighters, and unresolved-reference message helpers must not duplicate `element.getContainingFile() instanceof QinPsiFile`, because the PSI tree adapter owns element-to-file platform facts.
- `QinPsiReferences.findReferenceAt` should ask `QinPsiTree.elementAt` for raw offset-to-leaf lookup before walking references. `QinPsiReferences` owns reference registry traversal, but not the `PsiFile.findElementAt` bridge.
- `QinPsiReferences.findReferenceAt` should ask `QinPsiTree.referenceRangeInFile` when comparing a `PsiReference` range with a file offset. `QinPsiReferences` owns registry traversal, but not the relative reference range to file range bridge.
- `QinPsiReferences.findReferenceAt` should walk up the PSI tree through `QinPsiTree.parent`, not raw `PsiElement.getParent()`, because parent traversal is an IntelliJ PSI tree-shape bridge.
- Qin references, object symbol lookup, and editor surfaces that already hold a `PsiElement`/`PsiFile` should obtain `Project` through `QinPsiTree.project`. `ResolveCache`, Java PSI lookup, imported-file PSI lookup, StubIndex membership checks, lookup handlers, and external annotators must not call `myElement.getProject()`, `element.getProject()`, `file.getProject()`, or `psiFile.getProject()` directly, because `QinPsiTree` owns PSI element to IntelliJ project bridging.
- Editor and run surfaces that need a `VirtualFile` from a `PsiFile`, including external annotators, run gutter providers, run configuration producers, StubIndex filters, and source-file bridges, should use `QinPsiTree.virtualFile` or `QinPsiTree.originalVirtualFile`. They must not call `file.getVirtualFile()`, `psiFile.getVirtualFile()`, or `file.getOriginalFile().getVirtualFile()` directly outside the shared PSI tree bridge.
- Java run surfaces such as gutter icons and run configuration producers should consume a shared run-target PSI helper for Java `main`/test method detection and Qin project-path lookup. They must not duplicate `PsiTreeUtil`, method signature checks, annotation checks, or `QinProjectLocator` calls in each run UI surface.
- Run and test configurations should consume a shared defaults helper for resolved Qin project paths and main classes. Individual `RunConfiguration` implementations should not call `QinProjectLocator` directly, because default inference belongs to the run-configuration boundary rather than each UI/config class.
- Legacy Qin run/test configuration cleanup should live in a run-configuration helper. Startup should invoke that helper instead of owning `RunManager`, legacy type ids, or `removeConfiguration` loops.
- Run and test profile states should consume a shared command-line helper for `qin run` and `qin test` argument construction. Process/console states should not call `QinCommandResolver` directly or duplicate `run`, `test`, `--debug`, `--teamcity`, class, method, JVM, or program-argument assembly, because command-line shape belongs to the run boundary helper.
- IDEA background and tool-window surfaces that launch Qin CLI processes should consume a shared process-builder helper for BSP, compile, sync, forced sync, workspace sync, and configured script task command shapes. Surfaces such as project sync, Java file watching, startup sync, BSP clients, and the tool window should not call `QinCommandResolver.createProcessBuilder` directly, because they own UI/background workflow, not Qin CLI argument contracts.
- Startup should orchestrate `QinProjectSync` for background sync and must not own unused qin sync process helpers, stream readers, or direct `QinCliProcessBuilders` calls. Qin CLI process execution belongs to the sync/process-builder helpers, not to project startup.
- Tool-window project trees should consume the same Qin project discovery entry point as startup sync and run configuration defaulting. The tool window must not duplicate qin.config.js directory recursion, max-depth, hidden-directory, or excluded-directory rules, because project discovery belongs to the shared Qin project locator/discovery boundary.
- Qin project discovery should live in a shared helper such as `QinProjectDiscovery`. Startup, project sync, SDK defaults, run-configuration defaults, and tool-window project trees should consume that helper instead of routing discovery through `DebugStartup` or duplicating `LocalProjectResolver.scanAllProjects`.
- Qin project module file generation should live in a shared helper that owns `.iml` generation, source-folder repair, BSP classpath library entries, sources/javadoc jar probing, source-directory detection, and IDEA `modules.xml` registration. Startup, project sync, and tool-window surfaces should consume that helper instead of routing module-file work through startup or duplicating `.iml` XML construction.
- Qin project module metadata repair checks should live in the same module-file helper. Project sync may decide when to resync, but `.iml` LANGUAGE_LEVEL mutation, missing source-folder checks, BSP classpath entry validation, and `modules.xml` entry validation belong to `QinProjectModuleFiles`, not to `QinProjectSync`.
- Qin workspace SDK defaults should live in a shared helper that owns Qin config context detection, preferred Java version selection, and Java version string parsing. Startup SDK configuration should consume that helper and keep only orchestration logic.
- Project SDK selection and registration should live in a shared IDEA platform helper that owns configured JDK enumeration, `JavaSdk` version reads, best-match selection, and `JAVA_HOME` JDK registration through `ProjectJdkTable`. Startup SDK configuration should ask that helper for an SDK instead of owning IntelliJ JDK table or `JavaSdk.createJdk` calls directly.
- Project SDK persistence should live in a shared IDEA platform helper that owns `ProjectRootManager.setProjectSdk`, `misc.xml` SDK writes, VFS refresh, and deferred project-structure/index refresh logging. Startup SDK configuration should consume that helper instead of owning persistence or refresh calls directly.
- Project startup should delegate SDK setup to a startup SDK configuration helper. `DebugStartup` should only detect Qin SDK context and schedule that helper; it must not own Project SDK selection, JAVA_HOME registration, persistence, refresh, or Java-version comparison logic.
- IDEA platform tests that count or reject registered Qin reference types should also use `QinPsiReferences.references`, so tests exercise the same shared reference boundary as production consumers instead of calling `ReferenceProvidersRegistry` directly.
...
- `this` qualifier checks should use `QinReferenceElements.isThisQualifier`. Object method references, object field references, and object-member completion must not hard-code `"this".equals(...)`, because the reference helper owns qualifier-level member-access facts.
- Qin member call-vs-field boundaries should use `QinReferenceElements.isFollowedByCallParenthesis`. Object method references, object field references, and unresolved-reference messages must not query `QinPsiTokenStream` directly for this reference-level syntax fact.
...
- Unresolved-reference messages should use `QinReferenceElements.referenceName` when looking up and rendering the unresolved token name. They must not read or concatenate raw PSI text for reference identifiers, because reference element normalization belongs to the shared reference helper.
- Qin reference implementations and import binding checks should use `QinReferenceElements.referenceName` when comparing or storing reference-token names. They must not read raw PSI text from reference elements, because leaf-to-wrapper normalization belongs to the shared reference helper.
- Qin reference implementations should use `QinReferenceElements.referenceRange` for their `PsiReference` range. They must not duplicate full-token `TextRange` construction, because reference element range ownership belongs beside reference element normalization.
- `QinPsiTokenStream` should use `QinReferenceElements.referenceElement` when mapping leaf tokens back to structured reference wrappers. It must not own the `REFERENCE_IDENTIFIER` token mapping itself.
- Token consumers such as `QinReferenceElements` and `QinPsiTokenStream` should read IntelliJ element types through `QinPsiTree.elementType`, not raw `PsiElement.getNode()`. Keep raw node access centralized in `QinPsiTree`, except for dedicated mutation boundaries such as `QinPsiRenames`.
- `QinReferenceElements` should ask `QinPsiTree.parent` for reference wrappers and member-access parents, not raw `PsiElement.getParent()`, because reference helpers normalize PSI tree-shape facts before applying Qin reference semantics.
- `QinReferenceElements` should read reference token text and length through `QinPsiTree.elementText` and `QinPsiTree.elementTextLength`, not raw `PsiElement.getText()` or `PsiElement.getTextLength()`, because reference name/range normalization is a helper layer over PSI-platform facts.
- `QinPsiTokenStream` should read token text through `QinPsiTree.elementText`, not raw `PsiElement.getText()`, because token stream collection is a PSI-platform bridge rather than a reference-name semantic helper.
- `QinPsiTokenStream` should check token leaves through `QinPsiTree.isLeaf`, not raw `PsiElement.getFirstChild()`, because token collection is walking IntelliJ PSI tree shape through the shared adapter.
- Qin `PsiReferenceContributor` implementations should register their reference-token patterns through `QinReferenceElements`. Individual Java, object, object-member, or alias contributors must not duplicate `PlatformPatterns.psiElement(QinTokenTypes...)` registration sets.
- Qin `PsiReferenceContributor` implementations should also build providers through `QinReferenceElements.referenceProvider`. The helper owns Qin-file filtering, leaf-to-wrapper reference element bridging, and empty-reference fallback; individual contributors should only declare their candidate predicate and reference constructor.
- Qin object reference contributors should use `QinReferenceElements.objectReferenceProvider` for the shared Java interop exclusion. Object, object-method, and object-field contributors must not duplicate `QinJavaReference.isJavaReferenceCandidate` checks.
...
- The local alias inside an import specifier is a local declaration, not a remote reference. In `import { Counter as C }` or `import { Greeter as G }`, the exported name may resolve to the remote Qin object or Java `PsiClass`, but the local alias inside the specifier must not resolve to that remote symbol. The usage `C.next()` or `G.greet()` resolves through the import table.
...
- Object/class reference rename handlers must not add local-alias no-op fallbacks. Local aliases should be excluded before reference creation, and alias renames should flow through `QinImportAliasReference`.
- Find Usages for a remote Qin object or Java class should include the exported import specifier name even when it has a local alias, for example `Counter` in `import { Counter as C }` and `Greeter` in `import { Greeter as G }`. It should not treat the local alias declaration itself as a remote usage, and tests should assert both the exported-name inclusion and local-alias exclusion.
...
- `QinParserDefinition` should resolve object member declaration identity through `QinSourceStructure.memberDeclarationAtNameOffset(offset, ObjectMemberKind)`. Do not call separate field/method declaration offset helpers from ParserDefinition, because the source-structure adapter owns member-kind lookup.
...
- Import alias lookup by local name should go through `QinPsiTree.importAliasSpecifierNamed`, which delegates to `QinSourceStructure.importAliasSpecifierNamed`. Do not match `ImportSpecifier.localName()` or call source-structure alias lookup directly inside `QinImportBindings`; `QinImportBindings` should only bridge the selected alias range back to PSI through `QinPsiTree.importAliasNameElement`.
- Import exported-name lookup by local alias should bridge through `QinPsiTree.importExportedNameElement`, not direct `findElementAt`, so exported and local import names share the same source-range-to-PSI adapter boundary.
- Parsing a `PsiFile` into `QinSourceStructure` should go through `QinPsiTree.sourceStructure`. Consumers such as `QinFileStubBuilder`, `QinImportBindings`, and `QinObjectSymbols` should not call `QinSourceStructure.parse(file.getText())` directly, because the PSI tree adapter owns the `PsiFile` to source-structure bridge.
- PSI element to containing file lookup should go through `QinPsiTree.containingFile` in import/object/source-structure and token-stream bridge consumers. `QinImportBindings`, `QinObjectSymbols`, and `QinPsiTokenStream` must not call `PsiElement.getContainingFile()` directly when preparing to bridge source-structure or token facts back to PSI.
...
- Object declaration lookup by name should bridge through `QinPsiTree.sourceObjectDeclarationNamed`. Consumers such as `QinObjectSymbols` must not call `QinSourceStructure.objectDeclarationNamed` directly, because PSI/source-structure lookup belongs to the shared PSI tree adapter.
...
- Qin object member declaration lookup by name should go through `QinPsiTree.sourceObjectMemberDeclarationNamed`, which delegates to `QinSourceStructure.ObjectDeclaration.memberDeclarationNamed` with `QinSourceStructure.ObjectMemberKind`. Do not match `MemberDeclaration.name()` or derive source-structure member kind from PSI token types inside `QinObjectSymbols`; `QinObjectSymbols` should only bridge the selected member range back to PSI.
- Qin object member lookup inside `QinSourceStructure` should expose `ObjectMemberKind`-aware member helpers instead of separate field/method declaration lookup helpers. Keep name and offset lookup flowing through `memberDeclarations(kind)` so field-vs-method selection has one shared source-structure path.
- Qin object member name lookup inside `QinSourceStructure` should use `memberNames(ObjectMemberKind)`. Do not expose separate `fieldNames()` or `methodNames()` convenience helpers, because callers should make field-vs-method selection through the shared kind parameter.
- Qin object member collection should go through `QinPsiTree.sourceObjectMemberDeclarations`, which delegates to `QinSourceStructure.ObjectDeclaration.memberDeclarations` and carries each member's `QinSourceStructure.ObjectMemberKind` through the PSI bridge. `QinSourceStructure` should preserve source order in that collection instead of rebuilding it from `fields()` and then `methods()`. `ObjectDeclaration` should keep member facts in the source-order `ObjectMemberDeclaration` list and derive `memberDeclarations(ObjectMemberKind)` / `memberNames(ObjectMemberKind)` from that list; do not store parallel field and method declaration lists as a second source of truth. Do not flatten `fields()` and `methods()` or rederive member kind from bridged PSI tokens inside `QinObjectSymbols`; the source-structure adapter owns member order, kind, and duplicate-name handling before consumers bridge ranges back to PSI. When `QinObjectSymbols` needs an object-member name for StubIndex verification or completion filtering, it should carry `MemberDeclaration.name()` from `QinSourceStructure` through its internal member element wrapper instead of reading the bridged PSI element text.
...
- Stub serialization should read object member names through `QinSourceStructure.ObjectDeclaration.memberNames(ObjectMemberKind)`. `QinFileElementType` should not call `fieldNames()` or `methodNames()` directly, because field-vs-method member access belongs to the shared source-structure adapter.
...
- Qin object-name StubIndex membership lookup should live behind `QinObjectNameStubIndex`. Consumers such as `QinObjectSymbols` should ask that helper whether an imported file contains an exported object name, then bridge the declaration through `QinPsiTree`, instead of owning `StubIndex.getElements` or `GlobalSearchScope.fileScope` calls.
- `VirtualFile` to `PsiFile` lookup for Qin PSI resolution should live behind `QinPsiTree`. Consumers such as `QinObjectSymbols` should ask `QinPsiTree` for the imported PSI file instead of calling `PsiManager.getInstance(...).findFile(...)` directly.
- Indexed `PsiFile` to `VirtualFile` lookup should live behind `QinPsiTree.virtualFile`. StubIndex helpers such as `QinObjectNameStubIndex` and `QinObjectMemberStubIndexes` may own index querying and file-scope filtering, but should not call `PsiFile.getVirtualFile()` directly when confirming an indexed result belongs to the requested file.
- `PsiFile` to original `VirtualFile` lookup should live behind `QinPsiTree.originalVirtualFile`. `QinModuleImportTable` owns module path resolution, but it should not directly call `PsiFile.getOriginalFile().getVirtualFile()` when obtaining the source file for relative imports.
...
- Object member StubIndex key selection and lookup should use the shared `QinObjectMemberStubIndexes` helper. `QinFileElementType` should call `keyFor` while indexing stubs, and consumers such as `QinObjectSymbols` should call the helper's object-qualified member lookup instead of duplicating field-vs-method `StubIndexKey` selection or `QinSourceStructure.objectMemberKey` construction.
...
- Aliased imported Qin object member completion should prove both member kinds. `C.next` and `C.value` should complete to the remote `METHOD_NAME` and `FIELD_NAME` PSI elements from the imported file, preserving the local alias qualifier instead of treating aliases as a method-only path.
- Qin object member completion context should live in a shared helper such as `QinObjectMemberCompletions`. `QinObjectMemberCompletionContributor` should only translate returned completion members into lookup elements; it must not own qualifier lookup, Java interop exclusion, object-member lookup decisions, or fallback lookup text extraction from PSI text. Completion lookup text should come from the source-structure member name carried by `QinObjectMemberCompletions`, while the PSI element remains attached for navigation.
...
- Qin native completion tests should prove the negative boundary for Java qualifiers, including aliased Java imports. In a file that imports `Greeter` or `Greeter as G` from `java:demo`, completion at `Greeter.g` or `G.g` must not leak Qin object members such as `value` or `next`; Java and TypeScript-compatible member completion belongs to Java PSI/LSP surfaces, not `QinObjectMemberCompletionContributor`.
...
- Qin annotators should set annotation ranges through `QinPsiTree.elementRange`, not raw `PsiElement.getTextRange()`, because `QinPsiTree` owns PSI element range reads for editor surfaces.
...
- Qin semantic highlight style facts should live in a shared helper such as `QinSymbolHighlights`. `QinSymbolHighlightAnnotator` should only ask that helper for declaration/reference text attributes and emit IntelliJ annotations; it must not own token-to-color or reference-type-to-color mappings.
- Qin semantic highlight annotators should not query raw `PsiElement.getNode()` for eligibility or token facts. They should use `QinPsiTree.isQinFile`, `QinSymbolHighlights`, `QinReferenceElements`, and `QinPsiReferences` so token/node ownership stays behind shared helpers.
- Aliased import semantic highlighting should preserve the same remote/local split as navigation and rename. The exported name in `import { Counter as C }` or `import { Greeter as G }` should highlight as the remote Qin object or Java class reference, while the alias should highlight as a local import alias symbol/reference. Java aliased static methods and static fields should still highlight from their resolved `QinJavaReference` targets.
...
- Smoke tests should fail if `QinUnresolvedReferenceInspection` owns Java, object-method, or object-field branches directly. The inspection must ask `QinUnresolvedReferenceMessages.messageFor`, matching the annotator boundary.
- Unresolved-reference annotations should flow through a single `QinUnresolvedReferenceAnnotator` that consumes `QinUnresolvedReferenceMessages.messageFor`. Do not keep separate Java, object-method, or object-field annotator registrations that can diverge from the local inspection surface.
- Unresolved-reference inspections should be verified separately from annotator highlighting. Missing Qin object methods, Qin object fields, imported Java classes, and static Java members must report through `QinUnresolvedReferenceInspection` with the same messages as `QinUnresolvedReferenceAnnotator`, proving both surfaces share `QinUnresolvedReferenceMessages`.
- Unresolved Java class reporting for aliased imports must target the exported import specifier name through `QinJavaImportTable.findForSpecifierElement`. For example, `import { MissingGreeter as G } from "java:demo"` should report `Unresolved Java class demo.MissingGreeter` on `MissingGreeter`, not on the local alias `G`.
- Unresolved Java static member reporting must be suppressed when the imported Java class itself is unresolved. For `MissingGreeter.greet()` or `import { MissingGreeter as G }` followed by `G.greet()`, report the missing class through the class/exported-name reference and do not add a second missing-member diagnostic for `greet`.
- Unresolved Java static member reporting must respect import aliases through the shared import table. For example, `import { Greeter as G } from "java:demo"` followed by `G.missing()` should report `Unresolved static Java member demo.Greeter.missing` from both the annotator and inspection, without treating `G` as the exported Java class name.
...
- IDEA plugin descriptor smoke must verify the native Qin platform entry points: parser definition, syntax highlighter, reference contributors, completion contributor, annotators, inspections, StubIndexes, typed completion trigger, and lookup Enter handler. Use fixed-count and exact-set assertions for Qin-owned descriptor entries, so extra local platform hooks cannot appear without an explicit architecture decision. Package smoke must also verify the corresponding implementation classes are present in the plugin jar, so missing registrations or missing packaged classes fail before a plugin zip is installed.
- Package smoke should parse the packaged `META-INF/plugin.xml` and verify every descriptor class attribute such as `implementation`, `implementationClass`, `factoryClass`, and `serviceImplementation` resolves to a class entry in the plugin jar. Do not rely only on a handwritten required-class list for registered plugin entry points.
- Package smoke should require the `QinSourceStructure` nested classes consumed by PSI/import/member StubIndex bridges, including import matches, object member declarations, object member index entries, and object member kind. These source-structure value types are part of the packaged IDEA adapter contract, not disposable parser internals.
...
- Local import alias usages such as `C.value` and `G.greet()` should also have Go To Declaration coverage through `myFixture.getElementAtCaret()`, proving Ctrl+Click reaches the local `IMPORT_ALIAS_NAME` declaration while member references continue to resolve through the imported Qin object or Java class.
- Imported Qin object fields should have the same aliased-import coverage as methods. `C.value` from `import { Counter as C }` must resolve and navigate to the remote `FIELD_NAME`, proving the alias table feeds the shared object-member reference path instead of a method-only branch.
...
- Renaming a remote Qin object method or field through an aliased import member reference, such as `C.next()` or `C.value`, should update only the member token and preserve the local alias qualifier `C`. Do not rewrite aliased member usages back to the exported object name during member rename.
- No-local-parser smoke should guard the critical Qin-owned Find Usages and Rename platform tests. It must keep coverage for `ReferencesSearch` on `this.` plus qualified member usages, and `RenameProcessor` on same-file and aliased-import member references, so those tests cannot quietly be replaced by reference-only or string-rewrite checks.
- Qin PSI references and named PSI declarations should share the same leaf-token rename helper, currently `QinPsiRenames`. Individual references or `QinNamedPsiElement` must not duplicate `LeafElement.replaceWithText` plumbing, because Rename should have one IntelliJ PSI mutation boundary. `QinPsiRenames` is the allowed owner of raw AST node access such as `element.getNode()` and `node.getFirstChildNode()` for this mutation boundary.
- Qin named PSI token to element-class mapping should live in `QinNamedPsiElement`. `QinParserDefinition.createElement` should delegate to that factory instead of owning separate `OBJECT_NAME`, `METHOD_NAME`, `FIELD_NAME`, or `IMPORT_ALIAS_NAME` branches, so named declaration PSI remains the single rename/refactoring adapter surface.
- Qin named PSI `getName()` should read the name identifier through `QinPsiTree.elementText`, not raw `PsiElement.getText()`, because named declarations are also PSI adapter surfaces over platform token text.
- Qin named PSI `getNameIdentifier()` should use `QinPsiTree.firstChild`, not raw `PsiElement.getFirstChild()`, because child lookup is another IntelliJ PSI tree-shape fact.
...
When `packages/qin-idea-plugin-debug` compiles shared Slime/Subhuti lexer sources directly, its Gradle source set must include every helper required by that lexer boundary, including `SubhutiLexerVocabulary`, `TokenFactory`, and `RegexUtils`. Do not make the IDEA plugin reimplement those helpers or weaken `QinLexicalScanner`; `QinLspCompletionPlatformTest` and `lspPluginPackageSmoke` are the acceptance gates for this source-set contract.
