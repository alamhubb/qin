...
- Qin references and object symbol lookup should obtain `Project` through `QinPsiTree.project`. `ResolveCache`, Java PSI lookup, imported-file PSI lookup, and StubIndex membership checks must not call `myElement.getProject()` or `element.getProject()` directly, because `QinPsiTree` owns PSI element to IntelliJ project bridging.
