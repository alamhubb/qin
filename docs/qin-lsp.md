...
- Parsing a `PsiFile` into `QinSourceStructure` should go through `QinPsiTree.sourceStructure`. Consumers such as `QinImportBindings` and `QinObjectSymbols` should not call `QinSourceStructure.parse(file.getText())` directly, because the PSI tree adapter owns the `PsiFile` to source-structure bridge.
