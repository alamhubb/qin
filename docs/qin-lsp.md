...
- Qin PSI references and named PSI declarations should share the same leaf-token rename helper, currently `QinPsiRenames`. Individual references or `QinNamedPsiElement` must not duplicate `LeafElement.replaceWithText` plumbing, because Rename should have one IntelliJ PSI mutation boundary.
