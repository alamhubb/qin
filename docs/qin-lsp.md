...
- Qin object member completion context should live in a shared helper such as `QinObjectMemberCompletions`. `QinObjectMemberCompletionContributor` should only translate returned PSI members into lookup elements; it must not own qualifier lookup, Java interop exclusion, or object-member lookup decisions.
