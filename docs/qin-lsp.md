...
- IDEA native completion may be used only for PSI-owned Qin symbols whose declarations are already represented in the Qin PSI tree. For example, `Counter.` and `this.` can complete object `FIELD_NAME` and `METHOD_NAME` entries from the same object PSI. Do not use IDEA `CompletionContributor` as a broad fallback for TypeScript/Volar completions or Java member completions.
