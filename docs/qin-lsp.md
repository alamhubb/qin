...
- Qin references, object symbol lookup, and editor surfaces that already hold a `PsiElement`/`PsiFile` should obtain `Project` through `QinPsiTree.project`. `ResolveCache`, Java PSI lookup, imported-file PSI lookup, StubIndex membership checks, and lookup handlers must not call `myElement.getProject()`, `element.getProject()`, or `file.getProject()` directly, because `QinPsiTree` owns PSI element to IntelliJ project bridging.
