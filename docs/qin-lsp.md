...
- PSI element to containing file lookup should go through `QinPsiTree.containingFile` in import/object/source-structure bridge consumers. `QinImportBindings` and `QinObjectSymbols` must not call `PsiElement.getContainingFile()` directly when preparing to bridge source-structure facts back to PSI.
