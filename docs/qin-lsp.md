...
- Import binding lookup should use `QinSourceStructure.ImportDeclaration.specifierAtNameOffset` for both exported import names and local aliases. Do not split `exportedNameRange` and `localNameRange` checks inside `QinImportBindings`, because the source-structure adapter owns named-import offset semantics.
...
- Import alias lookup by local name should use `QinSourceStructure.importAliasSpecifierNamed`. Do not match `ImportSpecifier.localName()` inside `QinImportBindings`; `QinImportBindings` should only bridge the selected alias range back to PSI.
...
- Qin object and member resolution should also bridge from `QinSourceStructure` name ranges back to `OBJECT_NAME`, `FIELD_NAME`, and `METHOD_NAME` PSI. Consumers such as `QinObjectSymbols`, references, and completion must not descend PSI trees to rediscover object or member declarations with their own scans.
...
- Qin object member declaration lookup by name should use `QinSourceStructure.ObjectDeclaration.fieldDeclarationNamed` and `QinSourceStructure.ObjectDeclaration.methodDeclarationNamed`. Do not match `MemberDeclaration.name()` inside `QinObjectSymbols`; `QinObjectSymbols` should only bridge the selected member range back to PSI.
