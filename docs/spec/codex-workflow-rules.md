...
- OVS syntax details belong in the canonical parser grammar document `D:\project\qkyproject\qinall\ovsjs\ovs\ovs-compiler\docs\OvsParser需求文档.md`. Qin docs and skills should point there rather than restating the full grammar; the current short reminder is that OVS props use class-member-style `tag(...)` arguments separated by semicolons, while commas and JS object literal props are wrong OVS source syntax.
