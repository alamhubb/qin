...
- OVS canonical props syntax is class-member style inside `tag(...)`, for example `div(class = "a"; style = "color:red"; onClick() { console.log(123) }) { div { 123 } }`. Entries are separated with semicolons, not commas, and include `name = expression`, boolean shorthand, and method-like handlers. JS object props such as `div({ class: "a" }) { ... }` are wrong OVS source syntax, not a supported alternate form and not a workaround for OVS parser/compiler defects.
