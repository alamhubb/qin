...
- 前端使用 Vue + OVS + CSSTS。
- 后端使用 Qin/TS decorator controller + QinDb + QinWeb。
- 当前方向是继续实现“前端直接 import 后端 controller，然后 Qin/QinWeb 编译期 lowering 成 RPC 调用”，最终用户代码不需要手写前端 proxy controller。
...
- `main/controllers/UserController.ts`
  ...
  - 方法体里直接调用数据库：
    ...
    - `db.insertJson(...)`
    - `db.deleteByIdJson(...)`
  - 使用 `QinWeb.jsonRaw(...)` 返回 HTTP JSON。
- `main/db/schema.ts`
  - 定义 `qin_demo_users` 表。
  - 使用 `QinDb.fromSystemProperties()`。
- `main/qin-web-class.ts`
  - 后端 decorator helper。
  - `useQinWebController(app, UserController)` 从 method decorator 元数据挂载路由。
- `app/controllers/UserController.js`
  - 当前临时前端 proxy controller。
  - 现在也使用 decorator class 写法，并在方法体里调用 `qinWebCall(...)`。
  - 这是过渡方案，下一步要用编译期 lowering 去掉这个文件。
- `app/qin-web-class.js`
  - 当前临时浏览器侧 decorator/qinWebCall helper。
  - 这是过渡方案。
...
.\qin.bat run com.qin.web.QinWebAppSmokeTestMain
.\qin.bat run com.qin.web.QinWebControllerDecoratorFullstackSmokeTestMain
...
实现 Qin/QinWeb 编译期 RPC lowering，使前端可以直接写：
...
- Qin/QinWeb 编译期应识别这个 import 是 server controller boundary。
