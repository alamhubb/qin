...
| IDEA is only an LSP client for these languages | `plugin.xml` registers `.qin`, `.ovs`, `.cssts` file types and one `platform.lsp.serverSupportProvider`; `LSP.md` records the no-local-parser policy. | Partly proven |
...
1. Add a workspace-wide Qin project inventory smoke that lists every relevant
   ...
   ownership.
2. Add a no-local-parser IDEA smoke that scans plugin sources and fails if
   `.qin`, `.ovs`, or `.cssts` parser/lexer/completion implementations are added
   outside the LSP provider.
3. Expand generated parser parity checks beyond metadata: run a small corpus
   through Java QinParser and generated TypeScript QinParser and compare success
   or failure classes.
4. Expand `.class` target smoke from one CLI sample to a small Qin backend corpus
   that includes imports, exports, classes, functions, and `object`.
5. Keep this audit updated whenever a requirement moves from "partly proven" to
