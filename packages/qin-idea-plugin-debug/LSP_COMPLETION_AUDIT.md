...
- JVM `.class` execution is proven by the current focused CLI corpus and
  ...
  and parsed `while` execution through JVM loop control flow, not by every Qin
...
1. Expand the unified class-declaration `.class` gate toward broader parsed Qin
   source coverage for mutable loop locals, `for`/`break`/`continue`, and richer
   class member variants.
