...
| Volar LSP provides completion, definition, symbols, and semantic tokens | `lspServerDiagnosticsSmoke` requests and asserts completion, definition, document symbols, and semantic tokens for `.qin`, `.ovs`, and `.cssts` fixtures. Each language project's `tests/test-language-server.ts` also asserts these four feature responses contain concrete TS/Qin/OVS/CSSTS fixture symbols. `lspVerificationMatrixSmoke` verifies those language tests keep the feature requests and content assertions in place. | Proven for current smoke fixtures |
