...
- JVM `.class` execution is proven by the current focused CLI corpus and
  ...
  loop locals plus assignment execution, and parsed `for`/`break`/`continue`
  loop execution, not by every Qin
...
1. Expand the unified class-declaration `.class` gate toward broader parsed Qin
   source coverage for richer class member variants and additional loop forms
   such as `do while` / `for...of`.
