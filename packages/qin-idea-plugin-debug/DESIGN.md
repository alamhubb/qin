...
1. 检测 Qin 项目（包含 `qin.config.js` 的目录）
2. 生成 `.iml` 模块文件
3. 配置源代码目录、输出目录和依赖
4. 注册模块到 IDEA 项目
...
| `output` | `qin.config.js` → `java.outputDir` |
...
| 常量 | 值 | 说明 |
|---|---|---|
| `CONFIG_FILE` | `qin.config.js` | 配置文件名 |
| `QIN_DIR` | `.qin` | 缓存目录 |
