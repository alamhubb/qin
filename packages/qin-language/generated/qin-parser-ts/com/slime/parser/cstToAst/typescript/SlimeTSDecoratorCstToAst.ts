import { com_slime_ast_Expression, com_slime_ast_Expression as Expression } from "../../../ast/Expression.ts";
import { com_slime_ast_nodes_misc_Decorator, com_slime_ast_nodes_misc_Decorator as Decorator } from "../../../ast/nodes/misc/Decorator.ts";
import { com_slime_parser_cstToAst_SlimeAstCreateUtils, com_slime_parser_cstToAst_SlimeAstCreateUtils as SlimeAstCreateUtils } from "../SlimeAstCreateUtils.ts";
import { com_slime_parser_cstToAst_SlimeCstToAstUtils, com_slime_parser_cstToAst_SlimeCstToAstUtils as SlimeCstToAstUtils } from "../SlimeCstToAstUtils.ts";
import { com_subhuti_struct_SubhutiCst, com_subhuti_struct_SubhutiCst as SubhutiCst, com_subhuti_struct_SubhutiCst$Builder } from "../../../../subhuti/struct/SubhutiCst.ts";

// Generated by Qin TypeScript backend
import { __qin_builtin_constructor__, __qin_java_pattern_regexp__, __QinJavaLangString, __qin_java_class_info__, __qin_binary__, __qin_instanceof__, __qin_logical__, __QinJavaUtilArrayList, __QinJavaUtilUnmodifiableList, __QinJavaUtilList } from "@qin/java-sdk-js";

function __qin_structural_object__(value) {
  if (value == null || typeof value !== "object" || Array.isArray(value)) return false;
  return value.__qin_structural_object__ === true || value.constructor == null || value.constructor === Object;
}
const ArrayList = __QinJavaUtilArrayList;
class com_slime_parser_cstToAst_typescript_SlimeTSDecoratorCstToAst {
  __qin_field_transformer: com_slime_parser_cstToAst_SlimeCstToAstUtils | null = null as any;
  constructor(...__qin_args: any[]) {
    if (__qin_args.length === 1 && (__qin_args[0] === null || __qin_instanceof__(__qin_args[0], com_slime_parser_cstToAst_SlimeCstToAstUtils))) {
      const transformer: any = __qin_args[0];
      this.__qin_constructor_com_slime_parser_cstToAst_typescript_SlimeTSDecoratorCstToAst_1_0(transformer);
      return;
    }
    throw new Error("Unsupported Java constructor overload: SlimeTSDecoratorCstToAst/" + __qin_args.length);
  }
  __qin_constructor_com_slime_parser_cstToAst_typescript_SlimeTSDecoratorCstToAst_1_0(transformer: com_slime_parser_cstToAst_SlimeCstToAstUtils): void {
    this.__qin_field_transformer = null;
    this.__qin_field_transformer = transformer;
  }
  createDecoratorsAst(cst: com_subhuti_struct_SubhutiCst): any {
    let result: any = new __QinJavaUtilArrayList();
    this.collectDecorators(cst, result);
    return result;
  }
  createDecoratorAst(cst: com_subhuti_struct_SubhutiCst): any {
    let expression: any = null;
    for (const child of com_slime_parser_cstToAst_typescript_SlimeTSDecoratorCstToAst.safeChildren(cst)) {
      if (__qin_binary__("==", child, null)) {
        continue;
      }
      let name: any = child.getName();
      if ((__QinJavaLangString.equals("LeftHandSideExpression", name) || __QinJavaLangString.equals("CallExpression", name) || __QinJavaLangString.equals("MemberExpression", name) || __QinJavaLangString.equals("PrimaryExpression", name) || __QinJavaLangString.equals("IdentifierReference", name) || __QinJavaLangString.equals("Identifier", name))) {
        expression = this.__qin_field_transformer.createExpressionAst(child);
        break;
      }
    }
    if (__qin_binary__("==", expression, null)) {
      expression = com_slime_parser_cstToAst_SlimeAstCreateUtils.createIdentifier("decorator", cst.getLocation());
    }
    return new com_slime_ast_nodes_misc_Decorator(expression, com_slime_parser_cstToAst_SlimeAstCreateUtils.resolveSourceLocation(cst));
  }
  collectDecorators(cst: com_subhuti_struct_SubhutiCst, result: any): any {
    if (__qin_binary__("==", cst, null)) {
      return null;
    }
    if (__QinJavaLangString.equals("TSDecorator", cst.getName())) {
      result.add(this.createDecoratorAst(cst));
      return null;
    }
    for (const child of com_slime_parser_cstToAst_typescript_SlimeTSDecoratorCstToAst.safeChildren(cst)) {
      this.collectDecorators(child, result);
    }
    return null;
  }
  static safeChildren(cst: com_subhuti_struct_SubhutiCst): any {
    return (__qin_binary__("==", (__qin_binary__("==", cst, null) ? null : cst.getChildren()), null) ? __QinJavaUtilList.of() : (__qin_binary__("==", cst, null) ? null : cst.getChildren()));
  }
}
const SlimeTSDecoratorCstToAst = com_slime_parser_cstToAst_typescript_SlimeTSDecoratorCstToAst;

function run() {
  return null;
}

const __qinResult = run();
if (typeof globalThis !== 'undefined') {
  const __qinGlobal = globalThis as any;
  __qinGlobal.__qinResult = __qinResult;
}

export { com_slime_parser_cstToAst_typescript_SlimeTSDecoratorCstToAst };
