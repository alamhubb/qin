// Generated by Qin TypeScript backend
import { __qin_builtin_constructor__, __qin_java_pattern_regexp__, __QinJavaLangString, __qin_java_functional, __qin_java_class_info__, __qin_binary__, __qin_instanceof__, __qin_logical__, __qin_java_string_hash_code__, __qin_java_identity_hash_code__, __qin_java_value_hash_code__, __qin_java_values_equal__, __qin_java_hash_key__, __qin_java_hash_key_equals__, __QinJavaUtilObjects, __QinJavaUtilList, __QinJavaUtilUnmodifiableList } from "@qin/java-sdk-js";

function __qin_structural_object__(value) {
  if (value == null || typeof value !== "object" || Array.isArray(value)) return false;
  return value.__qin_structural_object__ === true || value.constructor == null || value.constructor === Object;
}
const Objects = __QinJavaUtilObjects;
class com_slime_java_ast_JavaAstNewExpression {
  __qin_field_typeName: string | null = null as any;
  __qin_field___qin_arguments: any = null as any;
  constructor(...__qin_args: any[]) {
    switch (__qin_args.length) {
      case 0: {
        this.__qin_constructor_com_slime_java_ast_JavaAstNewExpression_0();
        return;
      }
      case 2: {
        const typeName: any = __qin_args[0];
        const __qin_arguments: any = __qin_args[1];
        this.__qin_constructor_com_slime_java_ast_JavaAstNewExpression_2(typeName, __qin_arguments);
        return;
      }
      default: throw new Error("Unsupported Java constructor arity: JavaAstNewExpression/" + __qin_args.length);
    }
  }
  __qin_constructor_com_slime_java_ast_JavaAstNewExpression_0(): void {
    this.__qin_field_typeName = null;
    this.__qin_field___qin_arguments = null;
    __QinJavaUtilObjects.requireNonNull(this.__qin_field_typeName, "typeName cannot be null");
    __QinJavaUtilObjects.requireNonNull(this.__qin_field___qin_arguments, "arguments cannot be null");
    this.__qin_field___qin_arguments = __QinJavaUtilList.copyOf(this.__qin_field___qin_arguments);
  }
  __qin_constructor_com_slime_java_ast_JavaAstNewExpression_2(typeName: string, __qin_arguments: any): void {
    this.__qin_field_typeName = null;
    this.__qin_field___qin_arguments = null;
    (() => {
      this.__qin_field_typeName = typeName;
      this.__qin_field___qin_arguments = __qin_arguments;
      return null;
    })();
  }
  typeName(): any {
    return this.__qin_field_typeName;
  }
  __qin_arguments(): any {
    return this.__qin_field___qin_arguments;
  }
  equals(other) {
    if (this === other) return true;
    if (!__qin_instanceof__(other, com_slime_java_ast_JavaAstNewExpression)) return false;
    return __qin_java_values_equal__(this.__qin_field_typeName, other.__qin_field_typeName)
      && __qin_java_values_equal__(this.__qin_field___qin_arguments, other.__qin_field___qin_arguments);
  }
  hashCode() {
    let result = 1;
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_typeName);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field___qin_arguments);
    return result;
  }
  toString() {
    return "JavaAstNewExpression[" + "typeName=" + this.__qin_field_typeName + ", " + "arguments=" + this.__qin_field___qin_arguments + "]";
  }
}
com_slime_java_ast_JavaAstNewExpression.__qin_java_interfaces = ["com.slime.java.ast.JavaAstExpression"];
const JavaAstNewExpression = com_slime_java_ast_JavaAstNewExpression;

function run() {
  return null;
}

const __qinResult = run();
if (typeof globalThis !== 'undefined') {
  const __qinGlobal = globalThis as any;
  __qinGlobal.__qinResult = __qinResult;
}

export { com_slime_java_ast_JavaAstNewExpression };
