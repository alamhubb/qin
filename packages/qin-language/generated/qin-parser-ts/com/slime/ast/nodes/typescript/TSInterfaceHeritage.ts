import { com_slime_ast_AstNode, com_slime_ast_AstNode as AstNode } from "../../AstNode.ts";
import { com_slime_ast_AstNodeType, com_slime_ast_AstNodeType as AstNodeType } from "../../AstNodeType.ts";
import { com_slime_ast_SourceLocation, com_slime_ast_SourceLocation as SourceLocation } from "../../SourceLocation.ts";

// Generated by Qin TypeScript backend
import { __qin_builtin_constructor__, __qin_java_pattern_regexp__, __QinJavaLangString, __qin_java_class_info__, __qin_binary__, __qin_instanceof__, __qin_logical__, __qin_java_string_hash_code__, __qin_java_identity_hash_code__, __qin_java_value_hash_code__, __qin_java_values_equal__, __qin_java_hash_key__, __qin_java_hash_key_equals__, __qin_java_implements } from "@qin/java-sdk-js";

function __qin_structural_object__(value) {
  if (value == null || typeof value !== "object" || Array.isArray(value)) return false;
  return value.__qin_structural_object__ === true || value.constructor == null || value.constructor === Object;
}
class com_slime_ast_nodes_typescript_TSInterfaceHeritage {
  __qin_field_expression: com_slime_ast_AstNode | null = null as any;
  __qin_field_typeParameters: com_slime_ast_AstNode | null = null as any;
  __qin_field_location: com_slime_ast_SourceLocation | null = null as any;
  constructor(...__qin_args: any[]) {
    if (__qin_args.length === 3 && (__qin_args[0] === null || __qin_java_implements(__qin_args[0], "com.slime.ast.AstNode")) && (__qin_args[1] === null || __qin_java_implements(__qin_args[1], "com.slime.ast.AstNode")) && (__qin_args[2] === null || __qin_instanceof__(__qin_args[2], com_slime_ast_SourceLocation))) {
      const expression: any = __qin_args[0];
      const typeParameters: any = __qin_args[1];
      const location: any = __qin_args[2];
      this.__qin_constructor_com_slime_ast_nodes_typescript_TSInterfaceHeritage_3_0(expression, typeParameters, location);
      return;
    }
    throw new Error("Unsupported Java constructor overload: TSInterfaceHeritage/" + __qin_args.length);
  }
  __qin_constructor_com_slime_ast_nodes_typescript_TSInterfaceHeritage_3_0(expression: com_slime_ast_AstNode, typeParameters: com_slime_ast_AstNode, location: com_slime_ast_SourceLocation): void {
    this.__qin_field_expression = null;
    this.__qin_field_typeParameters = null;
    this.__qin_field_location = null;
    (() => {
      this.__qin_field_expression = expression;
      this.__qin_field_typeParameters = typeParameters;
      this.__qin_field_location = location;
      return null;
    })();
  }
  type(): com_slime_ast_AstNodeType {
    return com_slime_ast_AstNodeType.__qin_field_TS_INTERFACE_HERITAGE;
  }
  expression(): com_slime_ast_AstNode {
    return this.__qin_field_expression;
  }
  typeParameters(): com_slime_ast_AstNode {
    return this.__qin_field_typeParameters;
  }
  location(): com_slime_ast_SourceLocation {
    return this.__qin_field_location;
  }
  equals(other: any): boolean {
    if (this === other) return true;
    if (!__qin_instanceof__(other, com_slime_ast_nodes_typescript_TSInterfaceHeritage)) return false;
    const __qin_record_other: com_slime_ast_nodes_typescript_TSInterfaceHeritage = other;
    return __qin_java_values_equal__(this.__qin_field_expression, __qin_record_other.__qin_field_expression)
      && __qin_java_values_equal__(this.__qin_field_typeParameters, __qin_record_other.__qin_field_typeParameters)
      && __qin_java_values_equal__(this.__qin_field_location, __qin_record_other.__qin_field_location);
  }
  hashCode(): number {
    let result = 1;
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_expression);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_typeParameters);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_location);
    return result;
  }
  toString(): string {
    return ["TSInterfaceHeritage[", "expression=", this.__qin_field_expression, ", ", "typeParameters=", this.__qin_field_typeParameters, ", ", "location=", this.__qin_field_location, "]"].join("");
  }
}
com_slime_ast_nodes_typescript_TSInterfaceHeritage.__qin_java_interfaces = ["com.slime.ast.AstNode"];
const TSInterfaceHeritage = com_slime_ast_nodes_typescript_TSInterfaceHeritage;

function run() {
  return null;
}

const __qinResult = run();
if (typeof globalThis !== 'undefined') {
  const __qinGlobal = globalThis as any;
  __qinGlobal.__qinResult = __qinResult;
}

export { com_slime_ast_nodes_typescript_TSInterfaceHeritage };
