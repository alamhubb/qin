import { com_slime_ast_Expression, com_slime_ast_Expression as Expression } from "../../Expression.ts";
import { com_slime_ast_SourceLocation, com_slime_ast_SourceLocation as SourceLocation } from "../../SourceLocation.ts";
import { com_slime_ast_AstNodeType, com_slime_ast_AstNodeType as AstNodeType } from "../../AstNodeType.ts";

// Generated by Qin TypeScript backend
import { __qin_builtin_constructor__, __qin_java_pattern_regexp__, __QinJavaLangString, __qin_java_class_info__, __qin_binary__, __qin_instanceof__, __qin_logical__, __qin_java_string_hash_code__, __qin_java_identity_hash_code__, __qin_java_value_hash_code__, __qin_java_values_equal__, __qin_java_hash_key__, __qin_java_hash_key_equals__, __qin_java_implements, __QinJavaUtilArrayList, __QinJavaUtilUnmodifiableList } from "@qin/java-sdk-js";

function __qin_structural_object__(value) {
  if (value == null || typeof value !== "object" || Array.isArray(value)) return false;
  return value.__qin_structural_object__ === true || value.constructor == null || value.constructor === Object;
}
class com_slime_ast_nodes_misc_SwitchCase {
  __qin_field_test: com_slime_ast_Expression | null = null as any;
  __qin_field_consequent: __QinJavaUtilList<com_slime_ast_Statement> | null = null as any;
  __qin_field_location: com_slime_ast_SourceLocation | null = null as any;
  constructor(...__qin_args: any[]) {
    if (__qin_args.length === 3 && (__qin_args[0] === null || __qin_java_implements(__qin_args[0], "com.slime.ast.Expression")) && (__qin_args[1] === null || Array.isArray(__qin_args[1]) || __qin_instanceof__(__qin_args[1], __QinJavaUtilArrayList) || __qin_instanceof__(__qin_args[1], __QinJavaUtilUnmodifiableList)) && (__qin_args[2] === null || __qin_instanceof__(__qin_args[2], com_slime_ast_SourceLocation))) {
      const test: any = __qin_args[0];
      const consequent: any = __qin_args[1];
      const location: any = __qin_args[2];
      this.__qin_constructor_com_slime_ast_nodes_misc_SwitchCase_3_0(test, consequent, location);
      return;
    }
    throw new Error("Unsupported Java constructor overload: SwitchCase/" + __qin_args.length);
  }
  __qin_constructor_com_slime_ast_nodes_misc_SwitchCase_3_0(test: com_slime_ast_Expression, consequent: __QinJavaUtilList<com_slime_ast_Statement>, location: com_slime_ast_SourceLocation): void {
    this.__qin_field_test = null;
    this.__qin_field_consequent = null;
    this.__qin_field_location = null;
    (() => {
      this.__qin_field_test = test;
      this.__qin_field_consequent = consequent;
      this.__qin_field_location = location;
      return null;
    })();
  }
  type(): com_slime_ast_AstNodeType {
    return com_slime_ast_AstNodeType.__qin_field_SWITCH_CASE;
  }
  test(): com_slime_ast_Expression {
    return this.__qin_field_test;
  }
  consequent(): __QinJavaUtilList<com_slime_ast_Statement> {
    return this.__qin_field_consequent;
  }
  location(): com_slime_ast_SourceLocation {
    return this.__qin_field_location;
  }
  equals(other: any): boolean {
    if (this === other) return true;
    if (!__qin_instanceof__(other, com_slime_ast_nodes_misc_SwitchCase)) return false;
    const __qin_record_other: com_slime_ast_nodes_misc_SwitchCase = other;
    return __qin_java_values_equal__(this.__qin_field_test, __qin_record_other.__qin_field_test)
      && __qin_java_values_equal__(this.__qin_field_consequent, __qin_record_other.__qin_field_consequent)
      && __qin_java_values_equal__(this.__qin_field_location, __qin_record_other.__qin_field_location);
  }
  hashCode(): number {
    let result = 1;
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_test);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_consequent);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_location);
    return result;
  }
  toString(): string {
    return ["SwitchCase[", "test=", this.__qin_field_test, ", ", "consequent=", this.__qin_field_consequent, ", ", "location=", this.__qin_field_location, "]"].join("");
  }
}
com_slime_ast_nodes_misc_SwitchCase.__qin_java_interfaces = ["com.slime.ast.Misc", "com.slime.ast.AstNode"];
const SwitchCase = com_slime_ast_nodes_misc_SwitchCase;

function run() {
  return null;
}

const __qinResult = run();
if (typeof globalThis !== 'undefined') {
  const __qinGlobal = globalThis as any;
  __qinGlobal.__qinResult = __qinResult;
}

export { com_slime_ast_nodes_misc_SwitchCase };
