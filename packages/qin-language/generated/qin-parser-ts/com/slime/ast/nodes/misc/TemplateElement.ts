import { com_slime_ast_SourceLocation, com_slime_ast_SourceLocation as SourceLocation } from "../../SourceLocation.ts";
import { com_slime_ast_AstNodeType, com_slime_ast_AstNodeType as AstNodeType } from "../../AstNodeType.ts";

// Generated by Qin TypeScript backend
import { __qin_builtin_constructor__, __qin_java_pattern_regexp__, __QinJavaLangString, __qin_java_functional, __qin_java_class_info__, __qin_binary__, __qin_instanceof__, __qin_logical__, __qin_java_string_hash_code__, __qin_java_identity_hash_code__, __qin_java_value_hash_code__, __qin_java_values_equal__, __qin_java_hash_key__, __qin_java_hash_key_equals__ } from "@qin/java-sdk-js";

function __qin_structural_object__(value) {
  if (value == null || typeof value !== "object" || Array.isArray(value)) return false;
  return value.__qin_structural_object__ === true || value.constructor == null || value.constructor === Object;
}
class com_slime_ast_nodes_misc_TemplateElement {
  __qin_field_raw: string | null = null as any;
  __qin_field_cooked: string | null = null as any;
  __qin_field_tail: boolean | null = null as any;
  __qin_field_location: com_slime_ast_SourceLocation | null = null as any;
  constructor(...__qin_args: any[]) {
    switch (__qin_args.length) {
      case 4: {
        const raw: any = __qin_args[0];
        const cooked: any = __qin_args[1];
        const tail: any = __qin_args[2];
        const location: any = __qin_args[3];
        this.__qin_constructor_com_slime_ast_nodes_misc_TemplateElement_4(raw, cooked, tail, location);
        return;
      }
      default: throw new Error("Unsupported Java constructor arity: TemplateElement/" + __qin_args.length);
    }
  }
  __qin_constructor_com_slime_ast_nodes_misc_TemplateElement_4(raw: string, cooked: string, tail: boolean, location: com_slime_ast_SourceLocation): void {
    this.__qin_field_raw = null;
    this.__qin_field_cooked = null;
    this.__qin_field_tail = null;
    this.__qin_field_location = null;
    (() => {
      this.__qin_field_raw = raw;
      this.__qin_field_cooked = cooked;
      this.__qin_field_tail = tail;
      this.__qin_field_location = location;
      return null;
    })();
  }
  type(): any {
    return com_slime_ast_AstNodeType.__qin_field_TEMPLATE_ELEMENT;
  }
  raw(): any {
    return this.__qin_field_raw;
  }
  cooked(): any {
    return this.__qin_field_cooked;
  }
  tail(): any {
    return this.__qin_field_tail;
  }
  location(): any {
    return this.__qin_field_location;
  }
  equals(other) {
    if (this === other) return true;
    if (!__qin_instanceof__(other, com_slime_ast_nodes_misc_TemplateElement)) return false;
    return __qin_java_values_equal__(this.__qin_field_raw, other.__qin_field_raw)
      && __qin_java_values_equal__(this.__qin_field_cooked, other.__qin_field_cooked)
      && __qin_java_values_equal__(this.__qin_field_tail, other.__qin_field_tail)
      && __qin_java_values_equal__(this.__qin_field_location, other.__qin_field_location);
  }
  hashCode() {
    let result = 1;
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_raw);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_cooked);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_tail);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_location);
    return result;
  }
  toString() {
    return "TemplateElement[" + "raw=" + this.__qin_field_raw + ", " + "cooked=" + this.__qin_field_cooked + ", " + "tail=" + this.__qin_field_tail + ", " + "location=" + this.__qin_field_location + "]";
  }
}
com_slime_ast_nodes_misc_TemplateElement.__qin_java_interfaces = ["com.slime.ast.Misc", "com.slime.ast.AstNode"];
const TemplateElement = com_slime_ast_nodes_misc_TemplateElement;

function run() {
  return null;
}

const __qinResult = run();
if (typeof globalThis !== 'undefined') {
  const __qinGlobal = globalThis as any;
  __qinGlobal.__qinResult = __qinResult;
}

export { com_slime_ast_nodes_misc_TemplateElement };
