import { com_slime_ast_AstNode, com_slime_ast_AstNode as AstNode } from "../../AstNode.ts";
import { com_slime_ast_AstNodeType, com_slime_ast_AstNodeType as AstNodeType } from "../../AstNodeType.ts";
import { com_slime_ast_SourceLocation, com_slime_ast_SourceLocation as SourceLocation } from "../../SourceLocation.ts";

// Generated by Qin TypeScript backend
import { __qin_builtin_constructor__, __qin_java_pattern_regexp__, __QinJavaLangString, __qin_java_class_info__, __qin_binary__, __qin_instanceof__, __qin_logical__, __qin_java_string_hash_code__, __qin_java_identity_hash_code__, __qin_java_value_hash_code__, __qin_java_values_equal__, __qin_java_hash_key__, __qin_java_hash_key_equals__, __QinJavaUtilArrayList, __QinJavaUtilUnmodifiableList } from "@qin/java-sdk-js";

function __qin_structural_object__(value) {
  if (value == null || typeof value !== "object" || Array.isArray(value)) return false;
  return value.__qin_structural_object__ === true || value.constructor == null || value.constructor === Object;
}
class com_slime_ast_nodes_typescript_TSTypeParameterInstantiation {
  __qin_field_params: __QinJavaUtilList<com_slime_ast_AstNode> | null = null as any;
  __qin_field_location: com_slime_ast_SourceLocation | null = null as any;
  constructor(...__qin_args: any[]) {
    if (__qin_args.length === 2 && (__qin_args[0] === null || Array.isArray(__qin_args[0]) || __qin_instanceof__(__qin_args[0], __QinJavaUtilArrayList) || __qin_instanceof__(__qin_args[0], __QinJavaUtilUnmodifiableList)) && (__qin_args[1] === null || __qin_instanceof__(__qin_args[1], com_slime_ast_SourceLocation))) {
      const params: any = __qin_args[0];
      const location: any = __qin_args[1];
      this.__qin_constructor_com_slime_ast_nodes_typescript_TSTypeParameterInstantiation_2_0(params, location);
      return;
    }
    throw new Error("Unsupported Java constructor overload: TSTypeParameterInstantiation/" + __qin_args.length);
  }
  __qin_constructor_com_slime_ast_nodes_typescript_TSTypeParameterInstantiation_2_0(params: __QinJavaUtilList<com_slime_ast_AstNode>, location: com_slime_ast_SourceLocation): void {
    this.__qin_field_params = null;
    this.__qin_field_location = null;
    (() => {
      this.__qin_field_params = params;
      this.__qin_field_location = location;
      return null;
    })();
  }
  type(): com_slime_ast_AstNodeType {
    return com_slime_ast_AstNodeType.__qin_field_TS_TYPE_PARAMETER_INSTANTIATION;
  }
  params(): __QinJavaUtilList<com_slime_ast_AstNode> {
    return this.__qin_field_params;
  }
  location(): com_slime_ast_SourceLocation {
    return this.__qin_field_location;
  }
  equals(other: any): boolean {
    if (this === other) return true;
    if (!__qin_instanceof__(other, com_slime_ast_nodes_typescript_TSTypeParameterInstantiation)) return false;
    const __qin_record_other: com_slime_ast_nodes_typescript_TSTypeParameterInstantiation = other;
    return __qin_java_values_equal__(this.__qin_field_params, __qin_record_other.__qin_field_params)
      && __qin_java_values_equal__(this.__qin_field_location, __qin_record_other.__qin_field_location);
  }
  hashCode(): number {
    let result = 1;
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_params);
    result = result * 31 + __qin_java_value_hash_code__(this.__qin_field_location);
    return result;
  }
  toString(): string {
    return ["TSTypeParameterInstantiation[", "params=", this.__qin_field_params, ", ", "location=", this.__qin_field_location, "]"].join("");
  }
}
com_slime_ast_nodes_typescript_TSTypeParameterInstantiation.__qin_java_interfaces = ["com.slime.ast.AstNode"];
const TSTypeParameterInstantiation = com_slime_ast_nodes_typescript_TSTypeParameterInstantiation;

function run() {
  return null;
}

const __qinResult = run();
if (typeof globalThis !== 'undefined') {
  const __qinGlobal = globalThis as any;
  __qinGlobal.__qinResult = __qinResult;
}

export { com_slime_ast_nodes_typescript_TSTypeParameterInstantiation };
