...
import {
    ...
    createValueRegToken, createMode, type SubhutiCreateToken,
} from "subhuti";
...
/** 正则表达式模式 */
export const RegexpMode = createMode('regexp')

/** 模板字符串尾部模式（TemplateMiddle 和 TemplateTail） */
export const TemplateTailMode = createMode('templateTail')
...
/**
 * 正则表达式字面量的 pattern（不带 ^ 锚点，由 Lexer 统一添加）
 *
 * 根据 ECMAScript 规范：
...
export const ID_START_SOURCE = String.raw`[\p{ID_Start}$_]|\\u[0-9a-fA-F]{4}|\\u\{[0-9a-fA-F]+\}`

// IdentifierPart: UnicodeIDContinue | $ | \uXXXX | \u{XXXXX} | ZWNJ(\u200C) | ZWJ(\u200D)
export const ID_CONTINUE_SOURCE = String.raw`[\p{ID_Continue}$\u200C\u200D]|\\u[0-9a-fA-F]{4}|\\u\{[0-9a-fA-F]+\}`

// IdentifierName: IdentifierStart IdentifierPart*
export const IDENTIFIER_NAME_PATTERN = new RegExp(
...
export const SlimeJavascriptTokensObj = {
    ...
    // ============================================
    // A.1.12 模板字面量
    // 注意：模板字符串 token 已移至 SlimeContextTokensObj
    // 因为它们需要上下文相关的匹配（通过 token 插件控制）
    // ============================================
    ...
    // A.1.12 模板字面量
    // ============================================
    // NoSubstitutionTemplate 和 TemplateHead 以反引号开头，没有歧义
    ...
    // TemplateMiddle 和 TemplateTail 以 } 开头，与 RBrace 歧义，需要 TemplateTailMode
    TemplateMiddle: createEmptyValueRegToken(SlimeJavascriptTokenType.TemplateMiddle, /\}(?:[^`\\$]|\\[\s\S]|\$(?!\{))*\$\{/, TemplateTailMode),
    TemplateTail: createEmptyValueRegToken(SlimeJavascriptTokenType.TemplateTail, /\}(?:[^`\\$]|\\[\s\S]|\$(?!\{))*`/, TemplateTailMode),

    // ============================================
    // 正则表达式字面量
    // 以 / 开头，与 Slash 歧义，需要 RegexpMode
    // ============================================
    RegularExpressionLiteral: createEmptyValueRegToken(SlimeJavascriptTokenType.RegularExpressionLiteral, REGEXP_LITERAL_PATTERN, RegexpMode),
}
